def generate_scan_ids():
    ...
    scan_ids = [x for x in range(1001,1011)]
...
def filter_scan_times():
    ...
    long_scans = [x for x in scan_times if x> 10]
    return long_scans
...
def extract_patient_names():
    ...
    patient_names = [i['Name'] for i in patients]
...
def get_image_widths():
    ...
    widths = [i[0] for i in resolutions]
    return widths
...
def classify_suv_values():
    ...
    classifications = ["high" if i > 3.5 else "low" for i in suv_values]
...
def generate_report_titles():
    ...
    reports = [i+ " Report" for i in modalities]
    return reports
...
def filter_scan_durations():
    ...
    filtered_times = [i for i in scan_durations if i <= 30]
